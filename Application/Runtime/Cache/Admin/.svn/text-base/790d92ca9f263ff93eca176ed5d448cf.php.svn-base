...
<h1>
    ...
    <span id="search_id" class="action-span1"> - 修改角色表 </span>
    <div style="clear:both"></div>
</h1>
<div class="main-div">
    <form method="post" action="/Admin/Role/save/id/5" enctype="multipart/form-data" >
        ...
        <table cellspacing="1" cellpadding="3" width="100%">
            <input type="hidden" name="id" value="<?php echo $data['id']?>">            <tr>
                ...
                <td align="left">
                   <ul>
                       <?php foreach($pri as $p): if(strstr(','.$data["pri_id_list"].',',','.$p['id'].',')!==false){ $checked="checked='checked'"; }else{ $checked=''; } ?>
                       <li level="<?php echo $p['level']?>"><?php echo str_repeat('-',$p['level']*5)?><input type="checkbox" name='pri_id_list[]' <?php echo $checked;?> value="<?php echo $p['id']?>"><?php echo $p['pri_name']?></li>
                       <?php endforeach;?>
